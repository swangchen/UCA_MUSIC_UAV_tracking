...
for _ in range(iterations):
    for processed_channels in range(0, total_channels, 16):
        ...
        # 生成网格
        ang1_values = np.arange(1, 361)
        ang2_values = np.arange(1, 181)
        ang2, ang1 = np.meshgrid(ang2_values, ang1_values)  # 交换 ang1 和 ang2

        # 生成1/SP_C的三维数据
        Z = 1 / np.abs(SP_C)

        # 绘制三维图形
        ax.plot_surface(ang1, ang2, Z, cmap='viridis')

        # 设置图形的标题和坐标轴标签
        ax.set_title('SP功率谱')
        ax.set_xlabel('ang1')
        ax.set_ylabel('ang2')
        ax.set_zlabel('1/SP_C')
